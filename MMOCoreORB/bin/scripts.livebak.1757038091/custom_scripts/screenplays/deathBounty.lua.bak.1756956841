...
if (not deathBounty) then deathBounty = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not deathBounty.start) then
  function deathBounty:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'deathBounty' ====
if not deathBounty then deathBounty = ScreenPlay:new{ numberOfActs = 0 } end
if type(deathBounty.start) ~= "function" and type(start) ~= "function" then
  function deathBounty:start()
