...
if (not CorranHorn) then CorranHorn = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not CorranHorn.start) then
  function CorranHorn:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'CorranHorn' ====
if not CorranHorn then CorranHorn = ScreenPlay:new{ numberOfActs = 0 } end
if type(CorranHorn.start) ~= "function" and type(start) ~= "function" then
  function CorranHorn:start()
