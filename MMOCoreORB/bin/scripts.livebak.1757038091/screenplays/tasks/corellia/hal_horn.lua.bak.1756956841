...
if (not HalHorn) then HalHorn = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not HalHorn.start) then
  function HalHorn:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'HalHorn' ====
if not HalHorn then HalHorn = ScreenPlay:new{ numberOfActs = 0 } end
if type(HalHorn.start) ~= "function" and type(start) ~= "function" then
  function HalHorn:start()
