joz_jodhul_missions =
{
	{
		missionType = "assassinate",
		primarySpawns =
		{
			{ npcTemplate = "scavenger_rat", npcName = "*The Beast*" }
		},
		secondarySpawns =
		{

		},
		staticObjects =
		{

		},
		itemSpawns =
		{

		},
		rewards =
		{
			{ rewardType = "credits", amount = 50 }
		}
	}
}

npcMapJozJodhul =
{
	{
		spawnData = { npcTemplate = "joz_jodhul", x = 764.1, z = 423.3, y = 5866.2, direction = 143, cellID = 0, position = STAND },
		npcNumber = 1,
		stfFile = "@static_npc/corellia/joz_jodhul",
		missions = joz_jodhul_missions
	},
}

JozJodhul = ThemeParkLogic:new {
	npcMap = npcMapJozJodhul,
	className = "JozJodhul",
	screenPlayState = "joz_jodhul_quest",
	planetName = "corellia",
	distance = 3000,
}

registerScreenPlay("JozJodhul", true)

joz_jodhul_mission_giver_conv_handler = mission_giver_conv_handler:new {
	themePark = JozJodhul
}
joz_jodhul_mission_target_conv_handler = mission_target_conv_handler:new {
	themePark = JozJodhul
}

-- ===== AUTO-ADDED STUB (to prevent nil start() on load) =====
-- If the screenplay table doesn't exist, create a minimal one.
if (not JozJodhul) then JozJodhul = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not JozJodhul.start) then
  function JozJodhul:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'JozJodhul' ====
if not JozJodhul then JozJodhul = ScreenPlay:new{ numberOfActs = 0 } end
if type(JozJodhul.start) ~= "function" and type(start) ~= "function" then
  function JozJodhul:start()
    -- no-op: allows DirectorManager autostart to proceed without errors
    return true
  end
end
-- ==== /SAFETY ====
