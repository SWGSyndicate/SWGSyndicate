...
if (not BlkSunTasks) then BlkSunTasks = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not BlkSunTasks.start) then
  function BlkSunTasks:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'BlkSunTasks' ====
if not BlkSunTasks then BlkSunTasks = ScreenPlay:new{ numberOfActs = 0 } end
if type(BlkSunTasks.start) ~= "function" and type(start) ~= "function" then
  function BlkSunTasks:start()
