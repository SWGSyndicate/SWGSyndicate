...
if (not DenellKelVannon) then DenellKelVannon = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not DenellKelVannon.start) then
  function DenellKelVannon:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'DenellKelVannon' ====
if not DenellKelVannon then DenellKelVannon = ScreenPlay:new{ numberOfActs = 0 } end
if type(DenellKelVannon.start) ~= "function" and type(start) ~= "function" then
  function DenellKelVannon:start()
