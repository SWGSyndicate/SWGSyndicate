...
if (not CoralineDynes) then CoralineDynes = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not CoralineDynes.start) then
  function CoralineDynes:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'CoralineDynes' ====
if not CoralineDynes then CoralineDynes = ScreenPlay:new{ numberOfActs = 0 } end
if type(CoralineDynes.start) ~= "function" and type(start) ~= "function" then
  function CoralineDynes:start()
