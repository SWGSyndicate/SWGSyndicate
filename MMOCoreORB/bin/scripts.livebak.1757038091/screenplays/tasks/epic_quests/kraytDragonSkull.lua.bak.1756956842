...
if (not KraytDragonSkull) then KraytDragonSkull = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not KraytDragonSkull.start) then
  function KraytDragonSkull:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'KraytDragonSkull' ====
if not KraytDragonSkull then KraytDragonSkull = ScreenPlay:new{ numberOfActs = 0 } end
if type(KraytDragonSkull.start) ~= "function" and type(start) ~= "function" then
  function KraytDragonSkull:start()
