...
if (not Cx425Trooper) then Cx425Trooper = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not Cx425Trooper.start) then
  function Cx425Trooper:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'Cx425Trooper' ====
if not Cx425Trooper then Cx425Trooper = ScreenPlay:new{ numberOfActs = 0 } end
if type(Cx425Trooper.start) ~= "function" and type(start) ~= "function" then
  function Cx425Trooper:start()
