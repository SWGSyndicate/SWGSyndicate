...
if (not GinsDarone) then GinsDarone = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not GinsDarone.start) then
  function GinsDarone:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'GinsDarone' ====
if not GinsDarone then GinsDarone = ScreenPlay:new{ numberOfActs = 0 } end
if type(GinsDarone.start) ~= "function" and type(start) ~= "function" then
  function GinsDarone:start()
