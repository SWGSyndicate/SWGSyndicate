...
if (not VrakerOrde) then VrakerOrde = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not VrakerOrde.start) then
  function VrakerOrde:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'VrakerOrde' ====
if not VrakerOrde then VrakerOrde = ScreenPlay:new{ numberOfActs = 0 } end
if type(VrakerOrde.start) ~= "function" and type(start) ~= "function" then
  function VrakerOrde:start()
