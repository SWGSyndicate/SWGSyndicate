...
if (not CaptainEso) then CaptainEso = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not CaptainEso.start) then
  function CaptainEso:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'CaptainEso' ====
if not CaptainEso then CaptainEso = ScreenPlay:new{ numberOfActs = 0 } end
if type(CaptainEso.start) ~= "function" and type(start) ~= "function" then
  function CaptainEso:start()
