...
if (not OxilSarban) then OxilSarban = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not OxilSarban.start) then
  function OxilSarban:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'OxilSarban' ====
if not OxilSarban then OxilSarban = ScreenPlay:new{ numberOfActs = 0 } end
if type(OxilSarban.start) ~= "function" and type(start) ~= "function" then
  function OxilSarban:start()
