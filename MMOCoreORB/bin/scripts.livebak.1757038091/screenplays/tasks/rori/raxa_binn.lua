...
if (not RaxaBinn) then RaxaBinn = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not RaxaBinn.start) then
  function RaxaBinn:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'RaxaBinn' ====
if not RaxaBinn then RaxaBinn = ScreenPlay:new{ numberOfActs = 0 } end
if type(RaxaBinn.start) ~= "function" and type(start) ~= "function" then
  function RaxaBinn:start()
