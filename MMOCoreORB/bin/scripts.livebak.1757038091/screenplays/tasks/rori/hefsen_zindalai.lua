...
if (not HefsenZindalai) then HefsenZindalai = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not HefsenZindalai.start) then
  function HefsenZindalai:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'HefsenZindalai' ====
if not HefsenZindalai then HefsenZindalai = ScreenPlay:new{ numberOfActs = 0 } end
if type(HefsenZindalai.start) ~= "function" and type(start) ~= "function" then
  function HefsenZindalai:start()
