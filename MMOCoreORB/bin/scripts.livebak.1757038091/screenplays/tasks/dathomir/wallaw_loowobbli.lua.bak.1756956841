...
if (not WallawLoowobbli) then WallawLoowobbli = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not WallawLoowobbli.start) then
  function WallawLoowobbli:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'WallawLoowobbli' ====
if not WallawLoowobbli then WallawLoowobbli = ScreenPlay:new{ numberOfActs = 0 } end
if type(WallawLoowobbli.start) ~= "function" and type(start) ~= "function" then
  function WallawLoowobbli:start()
