...
if (not XaanTalmaron) then XaanTalmaron = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not XaanTalmaron.start) then
  function XaanTalmaron:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'XaanTalmaron' ====
if not XaanTalmaron then XaanTalmaron = ScreenPlay:new{ numberOfActs = 0 } end
if type(XaanTalmaron.start) ~= "function" and type(start) ~= "function" then
  function XaanTalmaron:start()
