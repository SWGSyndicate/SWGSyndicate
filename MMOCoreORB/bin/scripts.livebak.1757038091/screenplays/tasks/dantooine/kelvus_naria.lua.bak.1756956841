...
if (not KelvusNaria) then KelvusNaria = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not KelvusNaria.start) then
  function KelvusNaria:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'KelvusNaria' ====
if not KelvusNaria then KelvusNaria = ScreenPlay:new{ numberOfActs = 0 } end
if type(KelvusNaria.start) ~= "function" and type(start) ~= "function" then
  function KelvusNaria:start()
