...
if (not DrakkaJudarrl) then DrakkaJudarrl = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not DrakkaJudarrl.start) then
  function DrakkaJudarrl:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'DrakkaJudarrl' ====
if not DrakkaJudarrl then DrakkaJudarrl = ScreenPlay:new{ numberOfActs = 0 } end
if type(DrakkaJudarrl.start) ~= "function" and type(start) ~= "function" then
  function DrakkaJudarrl:start()
