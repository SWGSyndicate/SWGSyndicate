...
-- ==== SAFETY: auto-added guard for autostarted screenplay 'risArmorQuest' ====
if not risArmorQuest then risArmorQuest = ScreenPlay:new{ numberOfActs = 0 } end
if type(risArmorQuest.start) ~= "function" and type(start) ~= "function" then
  function risArmorQuest:start()
