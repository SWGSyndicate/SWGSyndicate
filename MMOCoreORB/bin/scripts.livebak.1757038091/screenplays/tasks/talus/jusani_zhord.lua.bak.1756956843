...
if (not JusaniZhord) then JusaniZhord = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not JusaniZhord.start) then
  function JusaniZhord:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'JusaniZhord' ====
if not JusaniZhord then JusaniZhord = ScreenPlay:new{ numberOfActs = 0 } end
if type(JusaniZhord.start) ~= "function" and type(start) ~= "function" then
  function JusaniZhord:start()
