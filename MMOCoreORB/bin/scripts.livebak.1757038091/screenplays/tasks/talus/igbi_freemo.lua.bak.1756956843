...
if (not IgbiFreemo) then IgbiFreemo = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not IgbiFreemo.start) then
  function IgbiFreemo:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'IgbiFreemo' ====
if not IgbiFreemo then IgbiFreemo = ScreenPlay:new{ numberOfActs = 0 } end
if type(IgbiFreemo.start) ~= "function" and type(start) ~= "function" then
  function IgbiFreemo:start()
