...
if (not HaleenSnowlineHagrinZeed) then HaleenSnowlineHagrinZeed = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not HaleenSnowlineHagrinZeed.start) then
  function HaleenSnowlineHagrinZeed:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'HaleenSnowlineHagrinZeed' ====
if not HaleenSnowlineHagrinZeed then HaleenSnowlineHagrinZeed = ScreenPlay:new{ numberOfActs = 0 } end
if type(HaleenSnowlineHagrinZeed.start) ~= "function" and type(start) ~= "function" then
  function HaleenSnowlineHagrinZeed:start()
