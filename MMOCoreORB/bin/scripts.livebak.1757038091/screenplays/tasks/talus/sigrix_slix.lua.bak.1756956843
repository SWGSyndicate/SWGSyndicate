...
if (not SigrixSlix) then SigrixSlix = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not SigrixSlix.start) then
  function SigrixSlix:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'SigrixSlix' ====
if not SigrixSlix then SigrixSlix = ScreenPlay:new{ numberOfActs = 0 } end
if type(SigrixSlix.start) ~= "function" and type(start) ~= "function" then
  function SigrixSlix:start()
