...
if (not NurlaSlinthiss) then NurlaSlinthiss = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not NurlaSlinthiss.start) then
  function NurlaSlinthiss:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'NurlaSlinthiss' ====
if not NurlaSlinthiss then NurlaSlinthiss = ScreenPlay:new{ numberOfActs = 0 } end
if type(NurlaSlinthiss.start) ~= "function" and type(start) ~= "function" then
  function NurlaSlinthiss:start()
