...
if (not RadleeMathiss) then RadleeMathiss = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not RadleeMathiss.start) then
  function RadleeMathiss:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'RadleeMathiss' ====
if not RadleeMathiss then RadleeMathiss = ScreenPlay:new{ numberOfActs = 0 } end
if type(RadleeMathiss.start) ~= "function" and type(start) ~= "function" then
  function RadleeMathiss:start()
