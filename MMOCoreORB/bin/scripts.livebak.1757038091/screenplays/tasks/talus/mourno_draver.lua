...
if (not MournoDraver) then MournoDraver = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not MournoDraver.start) then
  function MournoDraver:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'MournoDraver' ====
if not MournoDraver then MournoDraver = ScreenPlay:new{ numberOfActs = 0 } end
if type(MournoDraver.start) ~= "function" and type(start) ~= "function" then
  function MournoDraver:start()
