...
if (not EbennQ3Baobab) then EbennQ3Baobab = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not EbennQ3Baobab.start) then
  function EbennQ3Baobab:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'EbennQ3Baobab' ====
if not EbennQ3Baobab then EbennQ3Baobab = ScreenPlay:new{ numberOfActs = 0 } end
if type(EbennQ3Baobab.start) ~= "function" and type(start) ~= "function" then
  function EbennQ3Baobab:start()
