mullud_bombo_missions =
	{
		{
			missionType = "deliver",
			primarySpawns =
			{
				{ npcTemplate = "sentellus_gavrel", npcName = "Sentellus Gavrel" }
			},
			secondarySpawns = {},
			itemSpawns = 
			{
				{ itemTemplate = "object/tangible/mission/quest_item/mullud_bombo_q1_needed.iff", itemName = "Some survey equipment." }
			},
			rewards =
			{
				{ rewardType = "credits", amount = 25 },
			}
		},
	}

npcMapMulludBombo =
	{
		{
			spawnData = { npcTemplate = "mullud_bombo", x = 5045.9, z = -192, y = 6787.1, direction = -87, cellID = 0, position = STAND },
			worldPosition = { x = 5046, y = 6787 },
			npcNumber = 1,
			stfFile = "@static_npc/naboo/mullud_bombo",
			missions = mullud_bombo_missions
		}
	}

MulludBombo = ThemeParkLogic:new {
	npcMap = npcMapMulludBombo,
	className = "MulludBombo",
	screenPlayState = "mullud_bombo_quest",
	planetName = "naboo",
	distance = 600
}

registerScreenPlay("MulludBombo", true)

mullud_bombo_mission_giver_conv_handler = mission_giver_conv_handler:new {
	themePark = MulludBombo
}
mullud_bombo_mission_target_conv_handler = mission_target_conv_handler:new {
	themePark = MulludBombo
}
-- ===== AUTO-ADDED STUB (to prevent nil start() on load) =====
-- If the screenplay table doesn't exist, create a minimal one.
if (not MulludBombo) then MulludBombo = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not MulludBombo.start) then
  function MulludBombo:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'MulludBombo' ====
if not MulludBombo then MulludBombo = ScreenPlay:new{ numberOfActs = 0 } end
if type(MulludBombo.start) ~= "function" and type(start) ~= "function" then
  function MulludBombo:start()
    -- no-op: allows DirectorManager autostart to proceed without errors
    return true
  end
end
-- ==== /SAFETY ====
