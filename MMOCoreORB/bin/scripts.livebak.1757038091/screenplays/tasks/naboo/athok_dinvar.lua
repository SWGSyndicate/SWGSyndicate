...
if (not AthokDinvar) then AthokDinvar = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not AthokDinvar.start) then
  function AthokDinvar:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'AthokDinvar' ====
if not AthokDinvar then AthokDinvar = ScreenPlay:new{ numberOfActs = 0 } end
if type(AthokDinvar.start) ~= "function" and type(start) ~= "function" then
  function AthokDinvar:start()
