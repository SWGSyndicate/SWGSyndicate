...
if (not RakirBanai) then RakirBanai = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not RakirBanai.start) then
  function RakirBanai:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'RakirBanai' ====
if not RakirBanai then RakirBanai = ScreenPlay:new{ numberOfActs = 0 } end
if type(RakirBanai.start) ~= "function" and type(start) ~= "function" then
  function RakirBanai:start()
