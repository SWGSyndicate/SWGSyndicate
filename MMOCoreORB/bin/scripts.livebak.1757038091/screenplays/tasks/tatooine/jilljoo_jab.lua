...
if (not JilljooJab) then JilljooJab = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not JilljooJab.start) then
  function JilljooJab:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'JilljooJab' ====
if not JilljooJab then JilljooJab = ScreenPlay:new{ numberOfActs = 0 } end
if type(JilljooJab.start) ~= "function" and type(start) ~= "function" then
  function JilljooJab:start()
