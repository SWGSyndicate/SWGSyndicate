...
if (not IkkaGesul) then IkkaGesul = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not IkkaGesul.start) then
  function IkkaGesul:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'IkkaGesul' ====
if not IkkaGesul then IkkaGesul = ScreenPlay:new{ numberOfActs = 0 } end
if type(IkkaGesul.start) ~= "function" and type(start) ~= "function" then
  function IkkaGesul:start()
