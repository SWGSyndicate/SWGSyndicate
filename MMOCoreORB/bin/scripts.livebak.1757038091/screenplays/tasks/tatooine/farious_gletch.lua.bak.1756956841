...
if (not FariousGletch) then FariousGletch = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not FariousGletch.start) then
  function FariousGletch:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'FariousGletch' ====
if not FariousGletch then FariousGletch = ScreenPlay:new{ numberOfActs = 0 } end
if type(FariousGletch.start) ~= "function" and type(start) ~= "function" then
  function FariousGletch:start()
