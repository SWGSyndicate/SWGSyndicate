...
if (not LtHarburik) then LtHarburik = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not LtHarburik.start) then
  function LtHarburik:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'LtHarburik' ====
if not LtHarburik then LtHarburik = ScreenPlay:new{ numberOfActs = 0 } end
if type(LtHarburik.start) ~= "function" and type(start) ~= "function" then
  function LtHarburik:start()
