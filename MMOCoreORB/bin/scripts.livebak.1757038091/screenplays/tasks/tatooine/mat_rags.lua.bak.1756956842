...
if (not MatRags) then MatRags = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not MatRags.start) then
  function MatRags:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'MatRags' ====
if not MatRags then MatRags = ScreenPlay:new{ numberOfActs = 0 } end
if type(MatRags.start) ~= "function" and type(start) ~= "function" then
  function MatRags:start()
