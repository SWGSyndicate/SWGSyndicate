...
if (not NitraVendallan) then NitraVendallan = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not NitraVendallan.start) then
  function NitraVendallan:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'NitraVendallan' ====
if not NitraVendallan then NitraVendallan = ScreenPlay:new{ numberOfActs = 0 } end
if type(NitraVendallan.start) ~= "function" and type(start) ~= "function" then
  function NitraVendallan:start()
