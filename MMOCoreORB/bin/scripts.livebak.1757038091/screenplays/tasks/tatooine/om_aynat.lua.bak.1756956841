...
if (not OmAynat) then OmAynat = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not OmAynat.start) then
  function OmAynat:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'OmAynat' ====
if not OmAynat then OmAynat = ScreenPlay:new{ numberOfActs = 0 } end
if type(OmAynat.start) ~= "function" and type(start) ~= "function" then
  function OmAynat:start()
