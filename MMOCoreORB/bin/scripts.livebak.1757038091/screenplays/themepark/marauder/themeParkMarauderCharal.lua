...
if (not ThemeParkMarauderCharal) then ThemeParkMarauderCharal = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not ThemeParkMarauderCharal.start) then
  function ThemeParkMarauderCharal:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'ThemeParkMarauderCharal' ====
if not ThemeParkMarauderCharal then ThemeParkMarauderCharal = ScreenPlay:new{ numberOfActs = 0 } end
if type(ThemeParkMarauderCharal.start) ~= "function" and type(start) ~= "function" then
  function ThemeParkMarauderCharal:start()
