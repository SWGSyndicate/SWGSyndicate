...
if (not ThemeParkSmcZideera) then ThemeParkSmcZideera = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not ThemeParkSmcZideera.start) then
  function ThemeParkSmcZideera:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'ThemeParkSmcZideera' ====
if not ThemeParkSmcZideera then ThemeParkSmcZideera = ScreenPlay:new{ numberOfActs = 0 } end
if type(ThemeParkSmcZideera.start) ~= "function" and type(start) ~= "function" then
  function ThemeParkSmcZideera:start()
