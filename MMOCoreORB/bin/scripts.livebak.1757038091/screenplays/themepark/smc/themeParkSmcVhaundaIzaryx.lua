...
if (not ThemeParkSmcVhaundaIzaryx) then ThemeParkSmcVhaundaIzaryx = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not ThemeParkSmcVhaundaIzaryx.start) then
  function ThemeParkSmcVhaundaIzaryx:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'ThemeParkSmcVhaundaIzaryx' ====
if not ThemeParkSmcVhaundaIzaryx then ThemeParkSmcVhaundaIzaryx = ScreenPlay:new{ numberOfActs = 0 } end
if type(ThemeParkSmcVhaundaIzaryx.start) ~= "function" and type(start) ~= "function" then
  function ThemeParkSmcVhaundaIzaryx:start()
