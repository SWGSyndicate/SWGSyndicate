...
if (not HalloweenEventVendorLogic) then HalloweenEventVendorLogic = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not HalloweenEventVendorLogic.start) then
  function HalloweenEventVendorLogic:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====
