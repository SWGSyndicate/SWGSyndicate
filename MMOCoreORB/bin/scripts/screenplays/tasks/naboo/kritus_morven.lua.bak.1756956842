...
if (not KritusMorven) then KritusMorven = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not KritusMorven.start) then
  function KritusMorven:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'KritusMorven' ====
if not KritusMorven then KritusMorven = ScreenPlay:new{ numberOfActs = 0 } end
if type(KritusMorven.start) ~= "function" and type(start) ~= "function" then
  function KritusMorven:start()
