huff_zinga_missions =
	{
		{
			missionType = "assassinate",
			primarySpawns =
			{
				{ npcTemplate = "huff_blistmok", npcName = "Adult blistmok" }
			},
			secondarySpawns = {},
			itemSpawns = {},
			rewards =
			{
				{ rewardType = "credits", amount = 100 },
			}
		},
		{
			missionType = "assassinate",
			primarySpawns =
			{
				{ npcTemplate = "bocatt", npcName = "Bocatt" }
			},
			secondarySpawns = {},
			itemSpawns = {},
			rewards =
			{
				{ rewardType = "credits", amount = 100 },
			}
		},
		{
			missionType = "assassinate",
			primarySpawns =
			{
				{ npcTemplate = "sharptooth_langlatch", npcName = "Langlatch" }
			},
			secondarySpawns = {},
			itemSpawns = {},
			rewards =
			{
				{ rewardType = "credits", amount = 200 },
			}
		},
		{
			missionType = "assassinate",
			primarySpawns =
			{
				{ npcTemplate = "grassland_slice_hound", npcName = "Corellian Slice Hound" }
			},
			secondarySpawns = {},
			itemSpawns = {},
			rewards =
			{
				{ rewardType = "credits", amount = 200 },
			}
		},

	}

npcMapHuffZinga =
	{
		{
			spawnData = { npcTemplate = "huff_zinga", x = 5155, z = -192, y = 6636, direction = 47, cellID = 0, position = STAND },
			worldPosition = { x = 5155, y = 6636 },
			npcNumber = 1,
			stfFile = "@static_npc/naboo/huff_zinga",
			missions = huff_zinga_missions
		}
	}

HuffZinga = ThemeParkLogic:new {
	npcMap = npcMapHuffZinga,
	className = "HuffZinga",
	screenPlayState = "huff_zinga_quest",
	planetName = "naboo",
	distance = 600
}

registerScreenPlay("HuffZinga", true)

huff_zinga_mission_giver_conv_handler = mission_giver_conv_handler:new {
	themePark = HuffZinga
}
huff_zinga_mission_target_conv_handler = mission_target_conv_handler:new {
	themePark = HuffZinga
}
-- ===== AUTO-ADDED STUB (to prevent nil start() on load) =====
-- If the screenplay table doesn't exist, create a minimal one.
if (not HuffZinga) then HuffZinga = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not HuffZinga.start) then
  function HuffZinga:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'HuffZinga' ====
if not HuffZinga then HuffZinga = ScreenPlay:new{ numberOfActs = 0 } end
if type(HuffZinga.start) ~= "function" and type(start) ~= "function" then
  function HuffZinga:start()
    -- no-op: allows DirectorManager autostart to proceed without errors
    return true
  end
end
-- ==== /SAFETY ====
