...
if (not LebSlesher) then LebSlesher = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not LebSlesher.start) then
  function LebSlesher:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'LebSlesher' ====
if not LebSlesher then LebSlesher = ScreenPlay:new{ numberOfActs = 0 } end
if type(LebSlesher.start) ~= "function" and type(start) ~= "function" then
  function LebSlesher:start()
