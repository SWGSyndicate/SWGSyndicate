...
if (not VanaSage) then VanaSage = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not VanaSage.start) then
  function VanaSage:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'VanaSage' ====
if not VanaSage then VanaSage = ScreenPlay:new{ numberOfActs = 0 } end
if type(VanaSage.start) ~= "function" and type(start) ~= "function" then
  function VanaSage:start()
