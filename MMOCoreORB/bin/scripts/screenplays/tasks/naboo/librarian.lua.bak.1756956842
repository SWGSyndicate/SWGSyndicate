...
if (not Librarian) then Librarian = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not Librarian.start) then
  function Librarian:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'Librarian' ====
if not Librarian then Librarian = ScreenPlay:new{ numberOfActs = 0 } end
if type(Librarian.start) ~= "function" and type(start) ~= "function" then
  function Librarian:start()
