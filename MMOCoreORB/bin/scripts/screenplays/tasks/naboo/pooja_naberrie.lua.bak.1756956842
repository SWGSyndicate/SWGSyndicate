...
if (not PoojaNaberrie) then PoojaNaberrie = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not PoojaNaberrie.start) then
  function PoojaNaberrie:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'PoojaNaberrie' ====
if not PoojaNaberrie then PoojaNaberrie = ScreenPlay:new{ numberOfActs = 0 } end
if type(PoojaNaberrie.start) ~= "function" and type(start) ~= "function" then
  function PoojaNaberrie:start()
