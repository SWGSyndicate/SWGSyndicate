...
if (not Stella) then Stella = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not Stella.start) then
  function Stella:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'Stella' ====
if not Stella then Stella = ScreenPlay:new{ numberOfActs = 0 } end
if type(Stella.start) ~= "function" and type(start) ~= "function" then
  function Stella:start()
