...
if (not PfilbeeJhorn) then PfilbeeJhorn = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not PfilbeeJhorn.start) then
  function PfilbeeJhorn:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'PfilbeeJhorn' ====
if not PfilbeeJhorn then PfilbeeJhorn = ScreenPlay:new{ numberOfActs = 0 } end
if type(PfilbeeJhorn.start) ~= "function" and type(start) ~= "function" then
  function PfilbeeJhorn:start()
