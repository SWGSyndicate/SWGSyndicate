...
if (not AjuvaVanasterin) then AjuvaVanasterin = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not AjuvaVanasterin.start) then
  function AjuvaVanasterin:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'AjuvaVanasterin' ====
if not AjuvaVanasterin then AjuvaVanasterin = ScreenPlay:new{ numberOfActs = 0 } end
if type(AjuvaVanasterin.start) ~= "function" and type(start) ~= "function" then
  function AjuvaVanasterin:start()
