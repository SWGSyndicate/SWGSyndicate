...
if (not BootoLubble) then BootoLubble = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not BootoLubble.start) then
  function BootoLubble:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'BootoLubble' ====
if not BootoLubble then BootoLubble = ScreenPlay:new{ numberOfActs = 0 } end
if type(BootoLubble.start) ~= "function" and type(start) ~= "function" then
  function BootoLubble:start()
