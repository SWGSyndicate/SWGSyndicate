...
if (not StoosOlko) then StoosOlko = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not StoosOlko.start) then
  function StoosOlko:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'StoosOlko' ====
if not StoosOlko then StoosOlko = ScreenPlay:new{ numberOfActs = 0 } end
if type(StoosOlko.start) ~= "function" and type(start) ~= "function" then
  function StoosOlko:start()
