...
if (not NorenKrast) then NorenKrast = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not NorenKrast.start) then
  function NorenKrast:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'NorenKrast' ====
if not NorenKrast then NorenKrast = ScreenPlay:new{ numberOfActs = 0 } end
if type(NorenKrast.start) ~= "function" and type(start) ~= "function" then
  function NorenKrast:start()
