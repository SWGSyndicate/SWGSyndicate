...
if (not GarmBelIblis) then GarmBelIblis = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not GarmBelIblis.start) then
  function GarmBelIblis:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'GarmBelIblis' ====
if not GarmBelIblis then GarmBelIblis = ScreenPlay:new{ numberOfActs = 0 } end
if type(GarmBelIblis.start) ~= "function" and type(start) ~= "function" then
  function GarmBelIblis:start()
