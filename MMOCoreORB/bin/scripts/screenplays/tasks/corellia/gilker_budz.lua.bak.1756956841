...
if (not GilkerBudz) then GilkerBudz = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not GilkerBudz.start) then
  function GilkerBudz:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'GilkerBudz' ====
if not GilkerBudz then GilkerBudz = ScreenPlay:new{ numberOfActs = 0 } end
if type(GilkerBudz.start) ~= "function" and type(start) ~= "function" then
  function GilkerBudz:start()
