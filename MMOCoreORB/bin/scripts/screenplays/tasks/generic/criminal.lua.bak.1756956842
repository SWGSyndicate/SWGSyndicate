...
if (not GenericCriminal) then GenericCriminal = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not GenericCriminal.start) then
  function GenericCriminal:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'GenericCriminal' ====
if not GenericCriminal then GenericCriminal = ScreenPlay:new{ numberOfActs = 0 } end
if type(GenericCriminal.start) ~= "function" and type(start) ~= "function" then
  function GenericCriminal:start()
