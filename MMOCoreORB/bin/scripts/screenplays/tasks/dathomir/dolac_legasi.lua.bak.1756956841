...
if (not DolacLegasi) then DolacLegasi = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not DolacLegasi.start) then
  function DolacLegasi:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'DolacLegasi' ====
if not DolacLegasi then DolacLegasi = ScreenPlay:new{ numberOfActs = 0 } end
if type(DolacLegasi.start) ~= "function" and type(start) ~= "function" then
  function DolacLegasi:start()
