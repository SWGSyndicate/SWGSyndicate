...
if (not WardenVinzelHaylon) then WardenVinzelHaylon = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not WardenVinzelHaylon.start) then
  function WardenVinzelHaylon:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'WardenVinzelHaylon' ====
if not WardenVinzelHaylon then WardenVinzelHaylon = ScreenPlay:new{ numberOfActs = 0 } end
if type(WardenVinzelHaylon.start) ~= "function" and type(start) ~= "function" then
  function WardenVinzelHaylon:start()
