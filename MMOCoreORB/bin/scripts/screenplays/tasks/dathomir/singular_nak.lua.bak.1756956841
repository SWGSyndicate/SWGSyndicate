singular_nak_missions =
{
	...
	{ 
		...
		primarySpawns = 
		{ 
			{ npcTemplate = "nightsister_stalker_quest", npcName = "Nightsister Stalker" } --SORT OUT BETTER SingularNak
		}, 
		secondarySpawns = 
		{
		}, 
...
if (not SingularNak) then SingularNak = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not SingularNak.start) then
  function SingularNak:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'SingularNak' ====
if not SingularNak then SingularNak = ScreenPlay:new{ numberOfActs = 0 } end
if type(SingularNak.start) ~= "function" and type(start) ~= "function" then
  function SingularNak:start()
