...
if (not ticketTakerDS297) then ticketTakerDS297 = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not ticketTakerDS297.start) then
  function ticketTakerDS297:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'ticketTakerDS297' ====
if not ticketTakerDS297 then ticketTakerDS297 = ScreenPlay:new{ numberOfActs = 0 } end
if type(ticketTakerDS297.start) ~= "function" and type(start) ~= "function" then
  function ticketTakerDS297:start()
