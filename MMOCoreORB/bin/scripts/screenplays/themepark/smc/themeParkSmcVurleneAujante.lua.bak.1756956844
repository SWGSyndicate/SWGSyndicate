...
if (not ThemeParkSmcVurleneAujante) then ThemeParkSmcVurleneAujante = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not ThemeParkSmcVurleneAujante.start) then
  function ThemeParkSmcVurleneAujante:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'ThemeParkSmcVurleneAujante' ====
if not ThemeParkSmcVurleneAujante then ThemeParkSmcVurleneAujante = ScreenPlay:new{ numberOfActs = 0 } end
if type(ThemeParkSmcVurleneAujante.start) ~= "function" and type(start) ~= "function" then
  function ThemeParkSmcVurleneAujante:start()
