...
-- ==== SAFETY: auto-added guard for autostarted screenplay 'ThemeParkNym' ====
if not ThemeParkNym then ThemeParkNym = ScreenPlay:new{ numberOfActs = 0 } end
if type(ThemeParkNym.start) ~= "function" and type(start) ~= "function" then
  function ThemeParkNym:start()
