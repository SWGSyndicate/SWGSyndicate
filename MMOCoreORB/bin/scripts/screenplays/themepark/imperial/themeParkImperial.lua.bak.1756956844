...
if (not ThemeParkImperial) then ThemeParkImperial = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not ThemeParkImperial.start) then
  function ThemeParkImperial:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'ThemeParkImperial' ====
if not ThemeParkImperial then ThemeParkImperial = ScreenPlay:new{ numberOfActs = 0 } end
if type(ThemeParkImperial.start) ~= "function" and type(start) ~= "function" then
  function ThemeParkImperial:start()
