...
if (not ThemeParkRebel) then ThemeParkRebel = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not ThemeParkRebel.start) then
  function ThemeParkRebel:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'ThemeParkRebel' ====
if not ThemeParkRebel then ThemeParkRebel = ScreenPlay:new{ numberOfActs = 0 } end
if type(ThemeParkRebel.start) ~= "function" and type(start) ~= "function" then
  function ThemeParkRebel:start()
