...
if (not ThemeParkNightsister) then ThemeParkNightsister = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not ThemeParkNightsister.start) then
  function ThemeParkNightsister:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'ThemeParkNightsister' ====
if not ThemeParkNightsister then ThemeParkNightsister = ScreenPlay:new{ numberOfActs = 0 } end
if type(ThemeParkNightsister.start) ~= "function" and type(start) ~= "function" then
  function ThemeParkNightsister:start()
