...
if (not ThemeParkMarauderRaglithJorak) then ThemeParkMarauderRaglithJorak = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not ThemeParkMarauderRaglithJorak.start) then
  function ThemeParkMarauderRaglithJorak:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====

-- ==== SAFETY: auto-added guard for autostarted screenplay 'ThemeParkMarauderRaglithJorak' ====
if not ThemeParkMarauderRaglithJorak then ThemeParkMarauderRaglithJorak = ScreenPlay:new{ numberOfActs = 0 } end
if type(ThemeParkMarauderRaglithJorak.start) ~= "function" and type(start) ~= "function" then
  function ThemeParkMarauderRaglithJorak:start()
