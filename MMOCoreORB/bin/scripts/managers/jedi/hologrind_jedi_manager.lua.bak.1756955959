...
if (not HologrindJediManager) then HologrindJediManager = ScreenPlay:new { numberOfActs = 0 } end
-- If start() is missing, provide a no-op.
if (not HologrindJediManager.start) then
  function HologrindJediManager:start()
    return true -- no-op
  end
end
-- ===== /AUTO-ADDED STUB =====
